...
function loadStyle(href, callback) {
  for (var i = 0; i < document.styleSheets.length; i++) {
    if (document.styleSheets[i].href == href) {
      if (callback) callback(); // CSS ya cargado — llamar callback igualmente
      return;
    }
  }
...
(function ($) {
  ...
  var themeColorsDom =
    /*html cambio de color al set de colores*/
    `
...
`;
  ...
  var assetsBase = (typeof BASE_URL !== 'undefined') ? BASE_URL + 'assets/' : window.location.origin + '/assets/';
  loadStyle(assetsBase + theme, onStyleComplete);
